    ...
    >
      {chatHistory.length === 0 ? (
        <div className="m-auto text-center max-w-md">
          <h3 className="text-xl font-medium text-[var(--color-text)] mb-3 tracking-wide">费曼学习区</h3>
          <p className="text-[var(--color-text-muted)] text-sm leading-relaxed">请用你自己的话解释答案。系统将评估完整度、纠正错误概念，并针对薄弱环节进行追问。</p>
        </div>
      ) : (
        chatHistory.map((msg, idx) => (
          <div key={idx} className={`flex gap-5 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
            ...
            <div className={`p-5 rounded-2xl max-w-[85%] shadow-md backdrop-blur-md ${msg.role === 'user' ? 'bg-[rgba(15,23,42,0.03)] rounded-tr-none border border-[var(--color-border)]' : 'bg-[rgba(255,255,255,0.55)] border border-[var(--color-border)] rounded-tl-none'}`}>
              <div className="prose max-w-none prose-p:leading-relaxed prose-pre:border prose-pre:border-[var(--color-border)] text-sm md:text-base prose-headings:text-[var(--color-text)] prose-a:text-teal-700">
