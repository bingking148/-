...
from src.runtime_settings import clear_deepseek_api_key, get_deepseek_config_status, set_deepseek_api_key
...
def send_message(request, session_id):
    ...
    def event_generator():
        ...
        try:
            ...
            data = json.dumps({'content': str(e), 'node': 'system'}, ensure_ascii=False)
            yield f'data: {data}\n\n'
...
def manage_model_settings(request):
    if request.method == 'GET':
        return Response(get_deepseek_config_status())
    ...
    if request.method == 'PUT':
        ...
        payload = get_deepseek_config_status()
        payload['message'] = 'DeepSeek API Key saved'
        return Response(payload)

    clear_deepseek_api_key()
    payload = get_deepseek_config_status()
    payload['message'] = 'Custom DeepSeek API Key cleared'
    return Response(payload)
